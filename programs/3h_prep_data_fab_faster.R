...
#
# R Script: "3h_prep_data_fab_faster.R" 
...
save_stat <- function(x, name, digits = 0, big_mark = ",", dir = "output/other"){
  ...
  cat(formatted, file = file.path(dir, paste0(name, "_fab.tex"))) # fab = firm all bans
}

# # Cluster: Use CPUs Requested 
# dt_threads <- 8L
...
# Precompute these row-level components before grouping.
# This makes the grouped calculations simpler and more likely to use
# data.table's optimized aggregation paths.
agg2[, ave_exp_num_row := ave_exp * total_postings]
agg2[, ave_exp_den_row := fifelse(
  !is.na(ave_exp),
  total_postings,
  0
)]
...
agg2 <- agg2[
  ...
  .(
    ...
    ave_exp_num = sum(ave_exp_num_row, na.rm = TRUE),
    ave_exp_den = sum(ave_exp_den_row, na.rm = TRUE),
...
agg2[
  ,
  ave_exp_firm := fifelse(
    ave_exp_den == 0,
    NA_real_,
    ave_exp_num / ave_exp_den
  )
]

# Put descriptive variables beside their corresponding IDs
setcolorder(
  agg2,
  c(
...
agg2[
  ...
  `:=`(
    ...
    share_flextime = flextime_firm / total_postings_firm,
    
    treated = as.integer(
      !is.na(eff_inc1_year) & year >= eff_inc1_year
    ),
