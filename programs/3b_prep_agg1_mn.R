...
agg1_mn <- read_csv(file.path(data_raw, "sample_anastasi_agg1_v2.csv"))
...
n_drop_incb_a1_mn <- agg1_mn_treat %>%
...
n_drop_indb_a1_mn <- agg1_mn_treat %>%
  semi_join(
    states_ind_soc,
    by = c("state", "soc_4")
  ) %>%
...
n_drop_healthb_a1_mn <- agg1_mn_treat %>%
  semi_join(
    states_health_soc,
    by = c("state", "soc_4")
  ) %>%
...
n_drop_full_a1_mn <- agg1_mn_treat %>%
