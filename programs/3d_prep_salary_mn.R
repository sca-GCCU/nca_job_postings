...
# 1. Load data  
salary <- read_csv(file.path(data_raw, "anastasi_salary_v2.csv"))
...
n_drop_incb_sal_mn <- salary_mn_treat %>%
...
n_drop_indb_sal_mn <- salary_mn_treat %>%
  semi_join(
    states_ind_soc,
    by = c("state", "soc_4")
  ) %>%
...
n_drop_healthb_sal_mn <- salary_mn_treat %>%
  semi_join(
    states_health_soc,
    by = c("state", "soc_4")
  ) %>%
...
n_drop_full_sal_mn <- salary_mn_treat %>%
