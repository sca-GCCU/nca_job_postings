...
agg2_placebo_mn <- read_csv(file.path(data_raw, "anastasi_agg2_placebo.csv"))
...
agg2_placebo_mn <- agg2_placebo_mn %>%
  group_by(company) %>%
  mutate(
...
#agg2_placebo_mn_audit <- agg2_placebo_mn %>%
#  filter(
#    company == 3234293
#  )

#rm(agg2_placebo_mn_audit)

# Count number dropped 
n_drop_noise2 <- agg2_placebo_mn %>%
...
# Merge 
agg2_placebo_mn_treat <- agg2_placebo_mn %>%
...
agg2_placebo_mn_treat <- agg2_placebo_mn_treat %>%
  mutate(
...
states_inc1 <- agg2_placebo_mn_treat %>%
  group_by(state) %>%
  summarise(
...
#states_inc2 <- agg2_placebo_mn_treat %>%
#  group_by(state) %>%
#  summarise(
#    state_name = first(state_name),
...
states_hourly <- agg2_placebo_mn_treat %>%
  group_by(state) %>%
  summarise(
    state_name = first(state_name),
...
states_other <- agg2_placebo_mn_treat %>%
  group_by(state) %>%
  summarise(
    state_name = first(state_name),
...
states_ind <- agg2_placebo_mn_treat %>%
  group_by(state) %>%
  summarise(
    state_name = first(state_name),
...
states_health <- agg2_placebo_mn_treat %>%
  group_by(state) %>%
  summarise(
    state_name = first(state_name),
...
missing_soc <- states_ind_soc %>%
  anti_join(
    agg2_placebo_mn_treat %>% distinct(soc_4),
    by = "soc_4"
  ) 
...
n_drop_ind_ban2 <- agg2_placebo_mn_treat %>%
  semi_join(
    states_ind_soc,
    by = c("state", "soc_4")
  ) %>%
...
n_drop_health_ban2 <- agg2_placebo_mn_treat %>%
  semi_join(
    states_health_soc,
    by = c("state", "soc_4")
  ) %>%
...
states_full <- agg2_placebo_mn_treat %>%
  group_by(state) %>%
  summarise(
    state_name = first(state_name),
...
n_drop_full2 <- agg2_placebo_mn_treat %>%
...
# Data frames 
rm(agg2_placebo_mn, ind_crosswalk, state_nca_laws)
...
agg2_placebo_mn_analysis <- agg2_placebo_mn_treat %>%
  left_join(
    covariates_base,
    by = c("state"),
...
rm(agg2_placebo_mn_treat, covariates, covariates_base)
